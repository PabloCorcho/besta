...
class stellarTemplates:
    ...
    def __init__(self, templatesDir, logAgeArr, FeHArr):
        ...
        for ageIdx in range(self.nAges):
            for FeHIdx in range(self.nFeHs):
                self.hdf5Files.append(
                    templatesDir
                    + "templates-logT"
                    + str(self.logAgeArr[ageIdx])
                    + "-FeH"
                    + str(self.FeHArr[FeHIdx])
                    + ".hdf5"
                )
    ...
    def __readTemplates(self, ageIdx, FeHIdx):
        ...
        try:
            ...
            raise sys.exit(
                "\nERROR: One of the required datasets is missing in HDF5-file '"
                + fName
                + "'.\n"
            )
...
class powerlawIMFprior:
    ...
    def getWeightsDoublePL(self):
        ...
        for template in range(self.nTemplates):
            # Low mass end
            if (
                self.massLow[template] < self.Mbreak
                and self.massUp[template] < self.Mbreak
            ):
                self.weights[template] = (
                    (self.massLow[template]) ** (-self.alpha1 + 1)
                    - (self.massUp[template]) ** (-self.alpha1 + 1)
                ) / (self.alpha1 - 1)
            # High mass end
            elif (
                self.massLow[template] > self.Mbreak
                ...
                self.weights[template] = (
                    C2T
                    * (
                        (self.massLow[template]) ** (-self.alpha2 + 1)
                        - (self.massUp[template]) ** (-self.alpha2 + 1)
                    )
                    / (self.alpha2 - 1)
                )
            # Transition region
                ...
                self.weights[template] = (
                    (
                        (self.massLow[template]) ** (-self.alpha1 + 1)
                        - (self.Mbreak) ** (-self.alpha1 + 1)
                    )
                    / (self.alpha1 - 1)
                ) + C2T * (
                    (
                        (self.Mbreak) ** (-self.alpha2 + 1)
                        - (self.massUp[template]) ** (-self.alpha2 + 1)
                    )
                    / (self.alpha2 - 1)
                )
        ...
        if all(value == False for value in list(self.abVariations.values())):
            ...
            if hdf5File == None:
                ...
                raise sys.exit(
                    "\nERROR: HDF5-file '"
                    + hdf5File
                    + "' with response functions does not exist.\n"
                )
            else:
                ...
                if self.abVariations["Mg"]:
                    if respDex["MgDex"] >= 0.0:
                        MgRF = (
                            1.0
                            + respDex["MgDex"]
                            * (self.MgP[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
                    else:
                        MgRF = (
                            1.0
                            - respDex["MgDex"]
                            * (self.MgM[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
                ...
                if self.abVariations["Ca"]:
                    if respDex["CaDex"] >= 0.0:
                        CaRF = (
                            1.0
                            + respDex["CaDex"]
                            * (self.CaP[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
                    else:
                        CaRF = (
                            1.0
                            - respDex["CaDex"]
                            * (self.CaM[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
                ...
                if self.abVariations["Si"]:
                    if respDex["SiDex"] >= 0.0:
                        SiRF = (
                            1.0
                            + respDex["SiDex"]
                            * (self.SiP[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
                    else:
                        SiRF = (
                            1.0
                            - respDex["SiDex"]
                            * (self.SiM[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
                ...
                if self.abVariations["Ti"]:
                    if respDex["TiDex"] >= 0.0:
                        TiRF = (
                            1.0
                            + respDex["TiDex"]
                            * (self.TiP[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
                    else:
                        TiRF = (
                            1.0
                            - respDex["TiDex"]
                            * (self.TiM[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
                ...
                if self.abVariations["Na"]:
                    if respDex["NaDex"] >= 0.0:
                        NaRF = (
                            1.0
                            + respDex["NaDex"]
                            * (self.NaP[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
                    else:
                        NaRF = (
                            1.0
                            - respDex["NaDex"]
                            * (self.NaM[ageIndices[tIdx], FeHIndices[tIdx], :] - 1.0)
                            / 0.3
                        ).reshape((len(self.wavelength), 1))
