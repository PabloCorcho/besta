...
class invertCSP:
    ...
    def __MP(self, A, lam):
        ...
        try:
            wMP = np.linalg.solve(
                self.B + lam * self.C,
                np.dot(self.specTempT, self.flux) + lam * np.dot(self.C, self.w0),
            )
    ...
    def __bestLambda(self):
        ...
        if self.startIdx != 0 and self.startIdx != self.nStartValues - 1:
            s = scipy.optimize.brent(
                self.__minBayesianEvidence,
                brack=(
                    self.logLamStart[self.startIdx - 1],
                    self.logLamStart[self.startIdx],
                    self.logLamStart[self.startIdx + 1],
                ),
            )
        else:
            s = scipy.optimize.fmin(
                self.__minBayesianEvidence, self.startValue, disp=False
            )[0]
    ...
    def __findStartValue(self):
        ...
        for idx1 in range(self.nStartValues):
            self.minEvidence[self.nStartValues - 1 - idx1] = self.__minBayesianEvidence(
                self.logLamStart[self.nStartValues - 1 - idx1]
            )
    ...
    def __minBayesianEvidence(self, logLam):
        ...
        if (wMP >= 0).all():
            ...
            penaltyTerm = (
                0.5
                * lam
                * np.dot(np.dot(np.transpose(wMP - self.w0), self.C), (wMP - self.w0))
            )
            logDetA = np.linalg.slogdet(A)
            p1 = -chiSq - penaltyTerm - 0.5 * logDetA[1]
            p2 = self.BE - 0.5 * self.detCov
            p3 = (self.nTemplates / 2.0) * (np.log(lam)) + 0.5 * np.linalg.slogdet(
                self.C
            )[1]
    ...
    def BayesianEvidence(self):
        ...
        self.penaltyTerm = (
            ...
            * self.lam
            * np.dot(
                np.dot(np.transpose(self.wMP - self.w0), self.C), (self.wMP - self.w0)
            )
        )
        ...
        self.p3 = (self.nTemplates / 2.0) * (
            np.log(self.lam)
        ) + 0.5 * np.linalg.slogdet(self.C)[1]
