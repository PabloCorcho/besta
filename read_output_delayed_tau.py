...
obs_wl, obs_f, obs_e = np.loadtxt("BaseGM_mock_spectra_delayed_tau.dat", unpack=True)
...
ssp_data = np.loadtxt(
    "BaseGM_mock_spectra_delayed_tau.dat_ssp_model_spectra.dat", unpack=True)
...
#### STELLAR MASS
masses = norm_obs * mlr.flatten() * ssp_weights / ssp_weights.sum()
print("Masses: ", masses)

print("Weight sum: ", ssp_weights.sum())

masses = np.zeros((3, ssp_weights.size))
for n_ssp, n_mlr in zip(range(1, ssp_weights.size + 1), mlr.flatten()):
    masses[:, n_ssp - 1] = (
        norm_obs * n_mlr * pct_results[f"parameters--ssp{n_ssp}"][1],
        norm_obs * n_mlr * pct_results[f"parameters--ssp{n_ssp}"][2],
        norm_obs * n_mlr * pct_results[f"parameters--ssp{n_ssp}"][3]
                                 )
...
print(mass_history_edges)
mass_formed = mass_history_edges[:-1] - mass_history_edges[1:]

print("Recovered: ",recovered_mass_formed, "Total mass: ", np.log10(recovered_mass_formed.sum()))
